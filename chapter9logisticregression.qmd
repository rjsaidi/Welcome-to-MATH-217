...
names(heart) <- gsub(" ", "_", names(heart)) # removes spaces in spaces in variable names
```
...
```{r}
heart1 <- heart|>
  mutate(is_alive = ifelse(survived == "alive", 1, 0)) # alive = 1, died = 0
```
...
plot_log <- ggplot(data = heart1, aes(y = is_alive, x = age)) + 
  ...
  geom_smooth(method = "glm", color = "purple", method.args = list(family = "binomial"), se = FALSE)
plot_log
```
...
(odds)(is_alive) = 4.7797 + 0.9432(age).
...
```{r}
fit2 <- glm(is_alive ~ age + transplant, family = binomial(), data = heart1)
summary(fit2)
```

```{r}
age_70_tr <- data.frame(age = 70, transplant = "treatment")
age_70_cont <- data.frame(age = 70, transplant = "control")

augment(fit2, newdata = age_70_tr, type.predict = "response")
augment(fit2, newdata = age_70_cont, type.predict = "response")
```
...
```{r}
age_20_tr <- data.frame(age = 20, transplant = "treatment")
age_20_con <- data.frame(age = 20, transplant = "control")

augment(fit2, newdata = age_20_tr, type.predict = "response")
augment(fit2, newdata = age_20_con, type.predict = "response")
```
...
In this case, our model predicted that 96 patients would be not alive, and only 7 would be alive. Of those 96, 71 actually were not alive, while of the 7, 3 actually alive. Thus, our overall accuracy was 74 out of 103, or about 72%.
...
## Fit 3 includes interaction term

```{r}
fit3 <- glm(is_alive ~ age + transplant + age*transplant, family = binomial(), data = heart1)
summary(fit3)
```
...
*This is called "class imbalance", which can lead to biased results. A biased sample can often cause a great amount of disorder to the analysis.*
