...
```{r}
prop.test(x = 3, n = 62, conf.level = .95)
```
...
## Test Basic Assumptions Success-Failure Condition (SF Condition)
...
### The 95% confidence interval can be calculated as the sample proportion plus or minus two standard errors of the sample proportion - using the tidymodels functions.
...
se <- sqrt((p_hat_call*(1-p_hat_call))/n)
se
```
...
## count the s-f condition group sizes for GSS sample
...
## Using Tidymodels function to get the observed statistic (p-hat)
...
ggplot(null_dist, aes(x = stat)) +
  ...
  geom_text(x = 1/3, y = 9, color = "red", label = "null", size = 5)+
  geom_vline(xintercept = p_hat, color = "dodgerblue") + 
  geom_text(x = p_hat, y = 5, color = "dodgerblue", label = "p-hat", size = 5) +
  theme_bw()
```
...
1. State the question to study.
2. State the Ho and Ha (in sentences or symbolically)
3. Check the S-F condition (check the basic assumptions to perform the test)
4. Generate a null distribution and the observed statistics
5. Create a visual of the distribution (density plot or other) indicate the statistic as an x-intercept (vline)
6. Compute the p-value
7. two-part conclusion: 
- 1. Reject or fail to reject the null

- 2. Describe the level of evidence and context of the study with the results from the p-value.
