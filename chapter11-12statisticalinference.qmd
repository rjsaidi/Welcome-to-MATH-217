...
# only do this type of code ONE TIME
...
xbar to show sample mean
phat to show sample proportion
mu to show population mean

Ho: Prop of males not promoted is equal to the prop of females not promoted
Ha: Prop of males not promoted is different from the prop of females not promoted


tidymodels has 4 functions
...
decide_perm <- sex_disc |>
  # specify(y ~ x, y is categorical and x is also categorical)
...
## Conclusion based on p-value - 2 elements

1. Reject Ho because the p-value is less than 0.05.
2. There is **some** evidence that (restate Ha) **there is a difference in proportions of decisions to promote based on sex. **
...
Null is that there is a 50% chance for females to be promoted 
...
Find an interval of values that are plausible for the true parameter by calculating p-hatB12SE
