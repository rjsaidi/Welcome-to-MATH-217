...
nycflights |>
  ...
  summarize(sd_delay = sd(arr_delay))
```
...
aov_delay_origin <- aov(arr_delay ~ origin, data = nycflights)
...
The p-value of the ANOVA test is very small. Therefore we reject the null. There is very strong evidence that at least one mean arrival delay from one of the 3 airport origins is different.
