...
[![](cute%20possum.jpg){width="443"}](https://www.istockphoto.com/photos/cute-possum)
...
library(tidymodels)
library(openintro)
data(possum)  # function "data" only works for pre-built datasets from packages
```
...
## look at the distribution for head length
...
We can see a positive, moderate association: as head length increases, total body length tends to increase as well.
...
```{r}
fit1 <- lm(data = possum, total_l ~ head_l) #y~x
#tidy(fit1)  # lose many details from the model output that you get with "summary"
summary(fit1)
...
**\_\_\_\_\_% of the variation in the observations may be explained by this model.**
...
```{r}
ggplot(possum, aes(x=head_l, y=total_l, color = age))+
  geom_point() + # gradient can only be used to color numerical variable
...
ggpairs(possum2, c("site", "age", "head_l", "skull_w", "tail_l"))
# paired scatterplots of all the potential predictor variables
```
...
## EDA - Exploratory Plots of numerical and categorical variables

```{r}
ggplot(data = possum2, aes(age)) +
  geom_density()
```

```{r}
ggplot(data = possum2, aes(pop)) +
  geom_bar()
```

```{r}
ggplot(data = possum2, aes(factor(site))) +
  geom_bar()
```

```{r}
#options(scipen = 999)
possum2$site <- as.factor(possum2$site)
# convert something from categorical to numerical data$variable <- as.integer(data$variable)
full_model <- lm(data = possum2, total_l ~ site + pop + sex + age + head_l + skull_w + tail_l)
summary(full_model)
```
...
```{r}
# diagnostic plots
plot(full_model) # Base R
```

## Now look at the diagnostic plots

```{r}
library(ggfortify) # you do actually need library ggfortify to use the function autoplot()
...
```{r}
full_2 <- lm(data = possum2, total_l ~ site +  sex + head_l + skull_w + tail_l)
summary(full_2)
